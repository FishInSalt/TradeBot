...
**Architecture:** R3 rewrites PriceAlertService.check() with window-reset and removes cooldown cascade across 12+ files. R7 adds PriceLevelAlertInfo to BaseExchange with in-memory storage checked on each tick. R4 adds one-shot `_next_interval` to Scheduler + new agent tool. Implementation order: R3 → R7 → R4, all on single branch `feature/batch2-r3-r4-r7` (already created), merged as one PR.
...
In `_restore_session`, remove `alert_cooldown` variable and its reading from alert_data. Also remove `alert_cooldown_min=alert_cooldown` from the `WizardResult(...)` constructor call.
...
Update `config/settings_sim.yaml` alerts section: remove `cooldown_minutes`, set `window_minutes: 60`, `threshold_pct: 5.0`.
...
- In `test_set_price_alert_window_out_of_range`, add upper bound test (window=250 should return error):
  ```python
  result = await set_price_alert(deps, 3.0, 250, reasoning="test")
  assert "invalid" in result.lower() or "must be" in result.lower()
  ```

In `tests/test_wizard.py`: grep for `alert_cooldown_min` (8 occurrences) and remove from:
- `test_wizard_result_construction` — remove from WizardResult constructor
- `test_step_risk_scheduling_*` — remove from mock side_effects and assertions
- `test_run_wizard_full_flow` — remove from mock and assertion
- `test_run_wizard_*` — remove from WizardResult constructor in all variants
- `_show_summary` mock data — remove cooldown from alert string
...
1. Add `import uuid` to the top-level imports of `base.py`.

2. Add `PriceLevelAlertInfo` dataclass after `FillEvent`:
```python
@dataclass
...
3. Add `__init__` and 3 methods to `BaseExchange` (before the abstract methods):
```python
...
4. Add `super().__init__()` to `SimulatedExchange.__init__` (line 48) and `OKXExchange.__init__` (line 79).
...
Add `PriceLevelAlertInfo` to the existing top-level import from `base.py` (line 25 of app.py):
```python
from src.integrations.exchange.base import FillEvent, PriceLevelAlertInfo
```

Then in `run_agent_cycle`:
```python
...
- [ ] **Step 8: Add tool-layer tests for add_price_level_alert**
...
In `src/agent/trader.py`, add `from collections.abc import Callable` to the imports, then add to `TradingDeps` dataclass (after `approval_enabled`):
```python
...
- [ ] **Step 7: Add tool-layer tests for set_next_wake**
