...
- [ ] **Step 5: Verify alembic env.py loads (dry-run)**

Run: `uv run alembic current 2>&1 | head -5`
...
- [ ] **Step 2: Implement upgrade() — full body per spec §4.2**

Open the new migration file. Replace `upgrade()` body:
...
from src.storage.models import NAMING_CONVENTION
...
> **Note**: Do NOT modify `revision: str = "<auto>"` / `down_revision: ... = None` — alembic generated correct values when scaffolding. Only the `<auto>` placeholders in this plan are illustrative.

- [ ] **Step 3: Manual smoke — apply migration to a temp DB**

Run:

```bash
TMP=$(mktemp -d)
TRADEBOT_DB_URL="sqlite+aiosqlite:///$TMP/smoke.db" uv run python -c "
import asyncio
from src.storage.database import init_db

async def main():
    # init_db doesn't exist three-state yet (Task 6); for now manually run alembic via env.py
    pass

asyncio.run(main())
"
# Direct alembic CLI smoke (env.py CLI fallback path):
TRADEBOT_DB_URL="sqlite+aiosqlite:///$TMP/smoke.db" uv run alembic upgrade head
echo "--- Tables after upgrade ---"
sqlite3 "$TMP/smoke.db" ".schema decision_logs" | head -15
sqlite3 "$TMP/smoke.db" "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name='decision_logs';"
rm -rf "$TMP"
```

Expected:
- `alembic upgrade head` runs without exception
- `.schema decision_logs` shows `status VARCHAR(30) NOT NULL DEFAULT 'ok'` + `decision VARCHAR(20)`
- Index list includes `ix_decision_logs_session_id_cycle_id`
- (Migration ran on empty DB; no rows → no backfill applied; this just validates schema DDL)

If `alembic upgrade head` fails: typical cause is import error of `NAMING_CONVENTION`. Verify `from src.storage.models import NAMING_CONVENTION` at top of migration file works.
