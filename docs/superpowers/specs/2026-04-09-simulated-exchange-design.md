...
- `__init__(config, db_engine, session_id)` — 同步，仅保存配置和引用，不执行 I/O
- `async start()` — 异步初始化：查询 DB 恢复状态、REST 拉取种子 ticker、连接 WebSocket、启动撮合循环

`app.py` 工厂逻辑中：`exchange = SimulatedExchange(config, ...); await exchange.start()`。`OKXExchange` 的 `__init__` 也是纯同步的（仅创建 ccxt 客户端），两者风格一致。构造函数不属于 `BaseExchange` 接口，实例化由 `app.py` 根据 `exchange.name` 配置决定。
...
# 变更为
async def _update_trade_closed(deps, symbol, side, pnl, exit_price=None, fee=None)
```
...
```
SimulatedExchange.__init__(config, db_engine, session_id)   # 同步，仅保存配置
│
...
| `src/agent/tools_execution.py` | `_update_trade_closed()` 新增 `fee` 参数（累加：`record.fee = (record.fee or 0) + fee`）；`_record_trade_open()` 写入 `order.fee`；entry_price/exit_price/pnl 改用 Order.price 计算 |
