...
│  │                                           │   │              │   │
│  │  fetch_ticker()    create_order()         │   │  Agent 存储   │   │
│  │  fetch_ohlcv()     fetch_balance()        │   │ (agent 层     │   │
│  │  fetch_positions() set_leverage()         │   │  自行设计)    │   │
│  │  fetch_order()     fetch_open_orders()    │   │              │   │
│  │  fetch_closed_orders()                    │   │ DecisionLog  │   │
│  │  amount_to_precision()  close()           │   │ MemoryEntry  │   │
│  │                                           │   └──────────────┘   │
│  └──────────┬───────────────┬────────────────┘                      │
│             │               │                                       │
│     ┌───────┴───┐   ┌──────┴────────────────────────────────────┐  │
...
`create_order(symbol, side, order_type="market", amount)` 时同步处理。入口校验：symbol 匹配配置交易对、`amount > 0`，否则抛异常。根据当前持仓状态推断意图（对齐 OKX 净仓模式）：
