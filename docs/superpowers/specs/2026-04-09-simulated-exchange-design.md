...
TradeBot 是一个基于自主 agent 的交易系统，核心目标是**模拟真实世界交易员的交易流程**。Agent 扮演交易员角色，通过工具与交易所交互，自主完成市场分析、交易决策、仓位管理的完整循环。

### 交易员工作流

真实世界中，交易员的工作循环如下：

1. **触发** — 上一个决策的状态变化（如止损触发），或到了定时观察行情的时间
2. **信息收集** — 梳理当前行情、技术指标、资讯信息
3. **回顾历史** — 查看自己的交易日志（记录了历史决策细节及对应结果）
4. **做出决策** — 基于所有信息决定：开仓/平仓/设止损/观望
5. **等待** — 等待下一个触发时机

系统设计应围绕这个工作流：给 agent 提供尽可能多的 tools，让它能像真实交易员那样操作。

### 关键设计决策

**1. Agent 与交易所的交互方式应与真实交易所完全一致**

Agent 不应该依赖查询交易所的数据库来获取信息。它应该通过 `BaseExchange` 接口（等同于交易所 API）获取所有需要的数据：行情、余额、持仓、订单历史。这样切换真实交易所时 agent 的行为不需要任何改变。

**2. 模拟交易所应提供真实交易所能提供的所有数据**

包括实时行情（ticker、K线）、账户状态（余额、持仓）、订单管理（下单、查单、查历史）。Agent 查模拟交易所和查真实交易所的体验完全一致。

**3. 交易所与 Agent 的职责边界清晰**

| 职责 | 负责方 | 说明 |
|---|---|---|
| 实时状态（余额/持仓/行情/挂单） | 交易所 | Agent 通过 API 查询 |
| 订单处理和撮合 | 交易所 | 接收订单、撮合、返回结果 |
| 订单历史 | 交易所 | 记录所有订单的完整生命周期 |
| 交易决策和推理 | Agent | DecisionLog 记录每次决策 |
| 交易日志（决策+结果） | Agent | Agent 层自行设计（本 spec 不涉及） |
| 学习和记忆 | Agent | MemoryEntry 记录经验教训 |

**4. TradeRecord 的演进方向**

经过讨论，我们认为现有 TradeRecord 的设计（两步写入的交易生命周期跟踪）不是最优方案。更好的方式是让 agent 维护自己的交易日志（类似交易员的笔记本），记录决策及其结果。但这属于 agent 层设计，不在本 spec 范围内。本 spec 聚焦于模拟交易所模块，确保交易所能提供 agent 需要的所有数据。

### 为什么需要模拟交易所

直接用真实账户联调有两个问题：Bug 导致的错误交易需要手动取消、频繁交易产生手续费。本地模拟交易零风险零成本，Bug 直接删 DB 记录。模拟交易验证通过后，切换到真实账户只需改一个配置（`exchange.name: okx`）。
...
│  │                                           │   │ (agent 层     │   │
│  │  fetch_ticker()    create_order()         │   │  自行设计)    │   │
│  │  fetch_ohlcv()     fetch_balance()        │   │              │   │
│  │  fetch_positions() set_leverage()         │   │ DecisionLog  │   │
│  │  fetch_order()     fetch_open_orders()    │   │ MemoryEntry  │   │
│  │  fetch_closed_orders()                    │   │              │   │
...
2. 计算保证金和手续费：
   ...
   required = margin + fee
   ```
3. 校验余额：`free_usdt >= required` `[安全护栏]`，使用内部 free_usdt（不含 unrealized PnL），比 OKX 更严格——OKX 允许用未实现盈利作为保证金。Agent 基于 fetch_balance().free_usdt（含 unrealized PnL）计算下单量，因此 agent 自限的效果是安全的。边界情况：agent 有较大未实现盈利 + 高 position_pct 时可能被拒单，这是预期的保守行为。另注：上层 `open_position` 工具使用 `ticker.last` 估算下单数量，而实际成交价是 `ticker.ask`/`ticker.bid`，两者有微小差异（对齐真实交易所的下单估算误差）
4. 更新内部状态：
...
- **Agent 市价平仓**（`create_order`）：调用核心逻辑 + `_cancel_orphaned_orders`（仅在持仓完全移除后取消条件单，部分平仓不影响挂单，对齐真实交易所行为）
...
`create_order(symbol, side="sell", order_type="stop", amount=0.001, price=95000)` 时（同样在 `async with self._lock` 内）：
...
class FillEvent:
    ...
    position_side: str     # "long" / "short"（被平仓的持仓方向）
    trigger_reason: str    # "stop" / "take_profit" / "liquidation"（触发原因，agent 可据此调整反思策略）
    fill_price: float
    amount: float
    fee: float
    timestamp: int         # Unix milliseconds（对齐 OKX）
    # 注：不含 pnl 字段。交易所内部计算 PnL 用于余额更新，但不对外暴露。
    # 上层（agent 层）可从 fill_price + 自身持有的入场信息推导 PnL。
...
**时序约束**：fill_handler 的 try 块在 _matching_loop 迭代内 await 执行，期间 matching loop 无法处理新 tick。Agent 层的记录逻辑应尽量快速完成（如异步写 DB <10ms），重操作应在 agent cycle 内异步处理。
...
async def _scheduler_loop(self):
    ...
    # 注：首次 cycle 不检查 _running，stop() 期间需等待其完成（与现有行为一致）
...
| amount | float | 数量 |
| trigger_price | float, nullable | 触发价格（条件单必填，市价单/清算为 NULL） |
| status | str | open / closed / cancelled |
| filled_price | float, nullable | 实际成交价（成交时写入） |
| fee | float, nullable | 成交手续费（成交时写入） |
| filled_at | datetime, nullable | 成交时间（成交时写入） |
...
**Order.price 语义映射**（对齐 ccxt 行为）：

| Order.status | price 含义 | 对应 sim_orders 字段 |
|---|---|---|
...
├── 3. await self._ccxt.close()（关闭 ccxt Pro 实例，释放 WebSocket 连接和内部资源）
