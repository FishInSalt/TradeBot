...
**关键设计：所有订单统一走"下单 → 等待状态变化 → 异步通知"流程。**

`create_order` 的语义是 **agent 完成了向交易所下单的动作**，但订单的成交状态还不确定。无论市价单、限价单还是条件单，agent 的行为模式完全一致：

```
Agent 决策周期:
  1. Agent 做出交易决策
  2. 调用 create_order → 完成下单
  3. 写入 TradeAction（记录"我下了这个单"及理由）
  4. 周期结束，等待通知

订单状态变化（异步）:
  5. 订单成交/取消/过期 → FillEvent
  6. Handler 写入 TradeAction（记录"这个单成交了"及结果）
  7. 唤醒 Agent 进入新周期处理后续
```

市价单碰巧秒成交，限价单可能等几小时，条件单等价格触发——但从 agent 视角看，它永远不知道订单何时成交，只知道"我下了单"然后等通知。
...
5. **时序正确** — Tool 写入 TradeAction（T1）必然早于 FillEvent handler 写入（T2），数据时间线自然有序
...
| `src/integrations/exchange/simulated.py` | 修改 | `create_order()` 市价单 FillEvent 入队延迟处理；新增 `drain_pending_fills()`；实现 `cancel_order()`；FillEvent 改从 base.py 导入 |
...
async def get_trade_journal(deps: TradingDeps, limit: int = 20) -> str:
    ...
    # 3. 格式化输出
    lines = ["=== Trade Journal ==="]
    for a in reversed(actions):  # 按时间顺序（最旧在前）展示
...
- 新增 `reasoning` 参数
- `_check_approval` 签名需适配：去掉 `stop_loss` 和 `take_profit` 参数，新签名为 `_check_approval(deps, action, action_desc, position_pct, leverage)`。`ApprovalGate.check()` 接口同步变更
...
async def create_order(self, symbol, side, order_type, amount, price=None):
    ...
    async with self._lock:
        if order_type == "market":
            ...
            # 入队而非立即回调 — 由 app 层在 cycle 结束后统一处理
...
在 `run_agent_cycle` 结束后，drain 队列并处理：

```python
async def on_tick(trigger_type, context=None):
...
```
1. Tool 调用 create_order → 市价单成交 → FillEvent 入队（不回调）
2. Tool 写入 TradeAction("open_position") ← T1
3. 周期结束
4. on_tick finally: drain 队列 → handler 写入 TradeAction("order_filled") ← T2
5. handler 触发 scheduler → 新周期
T1 < T2 ✅ 时序正确
```
...
**Scheduler 单 pending 限制：** 当前 Scheduler 的 `trigger()` 方法仅保留一个 pending context。如果同一周期内产生多个 FillEvent，第二个 trigger 会将 `_pending_context` 覆盖为 None，导致所有 FillEvent 的 context 都丢失（最终 context=None），agent 唤醒时 prompt 中不会包含任何 FillEvent 详情。
...
两条记录通过相同的 `order_id` 关联，共同构成完整的决策 → 执行时间线。

**时序保证：** 市价单的 FillEvent 采用入队延迟处理（见 §SimulatedExchange 改造），Tool 写入 TradeAction 在先，FillEvent handler 写入在后，`created_at` 时间线自然有序，无需展示层特殊处理。
...
class MetricsService:
    ...
    async def compute(self, engine, session_id, current_position="none"):
        ...
        return _compute_metrics(pnls, self._initial_balance, current_position)
```
...
**接口变更：** `compute_from_trades(trade_records, current_position)` → `compute(engine, session_id, current_position)`。`initial_balance` 保留在构造器中（与当前代码一致）。MetricsService 变为异步，但不再需要访问交易所，只需查询 TradeAction 表。

## App 层改造

### run_agent_cycle

Prompt 构建需更新 FillEvent context 部分，追加 pnl 信息：

```python
# 当前（app.py:84-88）：
f"IMPORTANT EVENT: {context.trigger_reason} triggered "
f"— {context.symbol} {context.amount} @ {context.fill_price}"

# 改为：
msg = f"IMPORTANT EVENT: {context.trigger_reason} triggered — {context.symbol} {context.amount} @ {context.fill_price}"
if context.pnl is not None:
    msg += f", PnL: {context.pnl:.2f} USDT"
```

其余逻辑不变。TradeAction 的写入由各 tool 和 FillEvent handler 负责，不在 cycle 层面处理。
...
**迁移策略：** 由于系统处于早期开发阶段，不存在生产数据需要迁移。`create_all()` 只创建不存在的表，不会删除旧表（trade_records 会残留）。**用户需要在升级后删除旧数据库文件（`data/tradebot.db`）再重新运行**，由 `create_all()` 自动建立全新的表结构。
