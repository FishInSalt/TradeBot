...
**时序注意：** 由于 FillEvent callback 在 `create_order()` 内部被 await，order_filled 的 `created_at` 必然早于 open_position。`get_trade_journal` 展示时需按 order_id 分组，agent 主动操作（open_position/close_position）排在系统记录（order_filled）之前，避免"成交确认出现在开仓决策之前"的逻辑倒序。
