...
**关键设计：对齐真实交易所的行为模式。** 真实交易所中，两类事件的成交方式不同：

- **市价单** — 同步成交，`create_order()` 直接返回成交结果。交易员当场就知道成交价，在同一个决策周期内继续设 SL/TP
- **条件单（止损/止盈/强平）** — 异步触发，交易员不在场时由交易所自动执行。通过 FillEvent 唤醒交易员复盘

SimulatedExchange 对齐此行为：市价单在 `create_order()` 内同步成交并返回结果，不产生 FillEvent；条件单触发时通过 `_fill_callback` 产生 FillEvent。

#### 示例：完整的开仓到平仓流程

**Cycle 1 — 定时触发，agent 开仓并设风控（同一周期内完成）：**
...
Agent:
  ...
  7. open_position(side="long", position_pct=30, leverage=3)
     → 返回 "Position opened: long 0.0075 @ 60200, order: aaa"
     → TradeAction 记录：action="open_position"

  基于实际成交价 60200 计算风控价位：
     SL = 60200 × 0.97 = 58394
     TP = 60200 × 1.06 = 63812

  8. set_stop_loss(price=58394)
     → TradeAction 记录：action="set_stop_loss"
  9. set_take_profit(price=63812)
...
**FillEvent 仅用于异步事件** — 条件单触发（止损/止盈）和强平。这与真实交易所的行为一致：

- **市价单** — 同步成交，`create_order()` 直接返回结果。Agent 在同一决策周期内获取成交价并继续操作。**不产生 FillEvent。**
- **条件单/强平** — 异步触发，agent 不在场。通过 FillEvent 写入 TradeAction 并唤醒 agent 复盘。

SimulatedExchange 现有实现已符合此行为（`_fill_callback` 仅在 `_process_tick()` 中调用），**无需改造**。
...
async def _record_action_from_fill(engine, session_id, event: FillEvent):
    """将条件单/强平的 FillEvent 记录为 TradeAction。"""
...
2. 触发 Scheduler 的 conditional 唤醒（agent 被叫醒来复盘）
