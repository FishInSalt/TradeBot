...
```python
class MetricsService:
    async def compute(self, engine, session_id, initial_balance, current_position="none"):
...
**接口变更：** `compute_from_trades(trade_records, current_position)` → `compute(engine, session_id, initial_balance, current_position)`。MetricsService 变为异步，但不再需要访问交易所，只需查询 TradeAction 表。
