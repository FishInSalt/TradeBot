...
- 会话内容通过 `console.print()` 输出（app.py 32 处 + display.py 2 处 + approval.py 2 处），系统告警通过临时 Console 输出（okx.py 1 处）
...
当前 `console` 是 `app.py`、`display.py`、`approval.py` 的模块级全局变量（会话内容 36 处 + 系统告警 1 处）。`SessionConsole` 在运行时才创建，不能替代全局变量。迁移方式：
...
| `src/cli/display.py` | `display_metrics()` 增加 `console` 参数，删除模块级 `console`；删除死代码 `log_trade()` |
| `src/cli/approval.py` | `ApprovalGate` 构造函数增加 `console` 参数，删除模块级 `console` |
| `src/integrations/exchange/okx.py` | L138-139 临时 `Console().print()` 改为 `logger.warning()` |
| `main.py` | 新增 `--debug` |
| `.gitignore` | 新增 `logs/` |
...
- `position_sizing` 不在向导中暴露，使用默认值 `"percentage"`（PersonaConfig 中定义）
...
**凭证读取优先级**（向导 Step 1 实盘分支）：
1. `config/.credentials` 存在且有对应交易所 → 提示复用
2. `settings.exchange` 中有非空凭证（来自 `.env` 环境变量）→ 预填为默认值，用户确认或修改
3. 以上均无 → 提示手动输入
...
```python
# --- 新增字段（均有 Python 侧默认值，确保现有代码和测试不被破坏）---
...
ORM 默认值与迁移 SQL 的 DEFAULT 保持一致。`Session(name="test")` 等不传新字段的现有代码和测试不受影响。
...
**迁移策略**：幂等 `ALTER TABLE ADD COLUMN`。定义在 `session_manager.py` 中，在 `run()` Phase 2 的 `init_db()` 之后独立调用（不修改 `database.py`）：
...
恢复 "BTC sim #1"
  ...
  │     └── exchange_type == "okx" → 从 config/.credentials 加载凭证
  │           └── 文件不存在或凭证缺失 → 提示重新输入并保存
  ├── 模型选择:
  │     ├── 有 --model X → 直接使用 X（显式 flag 优先于历史记录）
  │     └── 无 --model →
  │           ├── 在 models.json 中查找 session 记录的 model_id
  │           │     ├── 找到 → "上次使用 claude-opus，继续？[Y/n]"
  │           │     │            └── N → 进入 wizard Step 3
  │           │     └── 未找到 → 提示 "上次使用的模型已不可用"，进入 wizard Step 3
  │           └── 用户选 N → 进入 wizard Step 3
  ├── status → "active"
  └── 返回 WizardResult
```
...
| 新建/恢复 + `--model X`，X 不存在 | 报错提示 "Model 'X' not found in models.json"，回退到 wizard Step 3 |
| 新建 session，无 `--model` | wizard Step 3 完整执行 |
| 恢复 session，无 `--model` | 提示确认是否继续使用上次模型 |

原则：显式 CLI flag 优先于 session 记录的历史值。模型 ID 无效时不直接退出，回退到交互选择。
...
首版仅支持同一时间运行一个 session。多个 OKX 实盘 session 共享同一交易所账户的持仓和余额，并行运行会导致仓位冲突。多 session 并行管理属 Phase 2 范围。
...
async def run(
    ...
    #   内部: 创建 scheduler(callback=on_tick)
    #         on_tick 闭包捕获 sc，调用 run_agent_cycle(..., console=sc)
    #         → 注册 fill/alert handler → exchange.start()
...
async def build_services(result: WizardResult, engine, session_id, sc, settings: Settings):
    ...
    approval_gate = ApprovalGate(
        ...
        timeout_seconds=settings.approval.timeout_seconds,  # 来自 settings.yaml
        console=sc,  # SessionConsole，审批面板写入 session 日志
    )
...
项目共 18 个测试文件（`test_*.py`），193 个测试。各 PR 对现有测试的影响：
...
| 文件 | 需求 | 改动 |
|------|------|------|
| `src/cli/app.py` | R5+R1+R2 | 重构 run() 为 6 阶段，删除 logging.basicConfig/console/scattered input。`TokenBudget` 类和 `_record_action_from_fill()` 保留在 app.py（分别由 Phase 5 和 Phase 6 使用） |
| `src/cli/display.py` | R5 | `display_metrics()` 增加 console 参数，删除模块级 console；删除死代码 `log_trade()` |
| `src/cli/approval.py` | R5 | `ApprovalGate` 增加 console 参数，删除模块级 console |
| `src/integrations/exchange/okx.py` | R5 | 临时 Console().print() 改为 logger.warning() |
...
| `src/integrations/exchange/simulated.py` | 接口不变 |
| `src/integrations/exchange/base.py` | 接口不变 |
| 各模块 `logger = logging.getLogger(__name__)` | root logger 配置自动生效 |
| `config/settings.yaml` | 内容不变，角色从"配置源"变为"唯一默认值模板" |
| `config/settings_sim.yaml` | 废弃，不再被读取。R5 实施时在文件头部添加 `# DEPRECATED` 注释 |
