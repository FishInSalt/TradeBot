...
定义在 `src/integrations/exchange/base.py`，与 `FillEvent` 同级（注：`AlertInfo` 定义在 `src/services/price_alert.py`，不在 `base.py`）：
...
注意：`SimulatedExchange.__init__` 和 `OKXExchange.__init__` 需在开头加 `super().__init__()`。
