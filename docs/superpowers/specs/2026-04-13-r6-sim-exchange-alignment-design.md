...
| D17 | open_position/close_position 的 pending 检查非对称 | open 不传 side（任何 pending 都阻止），close 传 side（只阻止同方向） | open 不传 side：防止 pending close 期间开仓导致复杂交叉订单（Agent 应等平仓 fill 确认后再决策）。close 传 side：pending open 不应阻止平仓（仓位当前存在，平仓请求合理） |
...
create_order("market", side, amount)
  └─ async with self._lock:
       ├─ ticker = self._latest_ticker; if None: raise RuntimeError
...
| `test_market_close_fill_event_has_pnl` | 除插入 tick 外，还需将 `drain_pending_fills()` 改为 `_fill_callback` 验证 |
| `test_market_order_unknown_type` | 现在 "limit" 不再是 unknown type。改为测试真正的 unknown type（如 "foobar"） |

**注**：所有使用 `drain_pending_fills()` 的测试统一改为 `_fill_callback` 验证（`drain_pending_fills` 将被删除）。

**测试 helper `_make_exchange`**（`test_simulated_exchange.py:8-27`）需新增 `exchange._frozen_usdt = 0.0` 初始化。
...
- `test_stop_order_without_price`（`price is None` 校验先于仓位检查，不受异步化影响）
...
| `src/agent/trader.py` | 注册 `place_limit_order` tool；更新 `get_open_orders` tool description（包含 market/limit） |
| `src/agent/tools_perception.py` | `get_open_orders` 返回值格式化：处理 `price=None`（pending market），区分 `[PENDING]`/`[LIMIT]`/`[STOP]` 标签 |
| `src/agent/persona.py` | 系统 prompt 变更（见下方草案） |
| **App** | |
...
现有 Decision Workflow 中的引导（persona.py:63-64）：
...
PR #1: 市价单异步化 + drain_pending_fills 清理
  ...
  tools_perception.py: get_open_orders 格式化处理 price=None + 类型标签
  trader.py: get_open_orders tool description 更新
  适配全部受影响测试 + 新增市价单异步测试
...
**`tools_perception.py` 格式化改动**（当前代码 `o.price:.2f` 对 `price=None` 会抛 TypeError）：
...
同时更新 `trader.py:65` 的 tool description：
- 旧：`"Get pending conditional orders (stop loss, take profit)."`
- 新：`"Get all pending orders (market awaiting fill, limit, stop loss, take profit)."`

### 测试回归风险

本次改动涉及 25 个测试的模式变更（23 个插入 tick 撮合步骤 + 2 个语义重写）。风险点：
