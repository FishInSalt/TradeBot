...
| 某个 API 不可用 / 超时 / 5xx | 该部分返回 "temporarily unavailable"，其余部分正常返回 |
...
| 任何 API 返回 HTTP 429 | `TTLCache` 将该条目 TTL 延长至 30min、返回 stale 缓存；若无缓存则降级 |

所有数据源均无需 API key，因此不存在"未配置 key"或"日配额耗尽"的分支——显著简化优雅降级的分支数量。
...
通过 `TTLCache.get_or_fetch` 统一处理：
- 数据源 client 遇到 HTTP 429 时抛 `RateLimitHit`
- `TTLCache` 捕获：如果该 cache key 有过期 data → 延长 TTL 至 30 分钟、返回 stale data、记录 WARNING
- 无 stale data 时 → 再抛出，caller（NewsService 方法）降级为返回空
- 其他错误（5xx / timeout）不经过 stale-cache 路径，直接降级
...
| 数据源 | 每日调用量 | 限额 |
|--------|----------|------|
| CoinDesk News | 48-96 tool calls（15min 缓存与 cycle 间隔对齐，每个 cycle 最多触发 1 次 API 调用。最坏情况 ~96/day，典型 ~48/day） | 无明确限额（pre-work 验证无限速迹象） |
| FGI | 4（6h 缓存） | 无限制 |
| ForexFactory | 1-4（6h 缓存） | 2 req/5min |
| OKX 公告 + 系统状态 | 48-144 × 2 端点（10min 缓存） | ~5 req/s（安全默认值） |
| ccxt funding/OI | 48-96 | 交易所 API 标准限额 |

**注意：** 采用单次 API 调用（不带 `categories` 过滤、`limit=20`）+ 本地分组方案后，缓存 key 仅含 `news_filter`（不含 symbol），且 TTL 与 cycle 间隔对齐（15min），每个 cycle 最多触发 1 次 API 调用。Agent 若交替使用不同 `news_filter` 值，会产生多个缓存条目，但由于 TTL 与 cycle 间隔一致，不会导致缓存穿透。
