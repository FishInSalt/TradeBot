...
**测试覆盖深度声明 — 早返回 vs 富文本路径**：

Iter 4 fact-only 新测试中部分工具走 **早返回路径**（如 `deps.X = None` → "service not configured." 单行；`deps.exchange.fetch_positions = []` → "No positions to close." 单行）：
- 早返回：`get_market_news` / `get_macro_context` / `get_etf_flows` / `get_stablecoin_supply` / `get_trade_journal` / `get_performance` / 5 执行工具早 return
- Happy-path（富文本）：`get_higher_timeframe_view`（250-bar OHLCV）/ `get_account_balance`（Balance 渲染）/ `get_open_orders`（OCO + limit）/ `get_active_alerts`（params + price levels）/ `get_market_data`（typical OHLCV + indicators）/ `get_price_pivots`（100-bar swing fixture）/ `open_position`（full mock chain）/ `get_position` (PR #22, 4 scenarios) / `get_order_book` / `get_recent_trades` / `get_multi_timeframe_snapshot`（PR #22）

早返回路径输出是几行字模板，wordlist 命中概率近零；这是**最低保障**而非"暴露最大风险面"。富文本路径才能实测渲染层是否漏 banned words。

**Trade-off 与缓解**：
- 不强制全部走 happy-path：富文本 mock 设置成本不均（macro/etf/news 服务返回结构复杂），plan 体量已大，YAGNI 倾向于先最低保障
- **观察期 follow-up**：观察期 LLM 实跑产生真实 tool 输出 log（`tool_calls.output_text`），作为富文本采样源回写 fact-only 测试（独立 PR）。比 mock-driven 测试更具诊断价值（覆盖真实 LLM trigger 的输入空间），与项目 observation-driven 哲学对齐
- 命中触发：观察期或日常使用中如 fact-only CI 红，即按红的工具补 happy-path mock
...
| `test_persona.py::test_prompt_contains_memory_quality_guidance` | **缩减** — 原断言两条：`"actionable"` + `"not worth saving"`（L48-49）。`"not worth saving"` 来自 L30（删除）失效；`"actionable"` 在新 L28 `Save actionable lessons to memory.` 保留 — 缩减为只断言 `"actionable"`，守护 L28 drift。函数数不变 |
...
- **Persona 测试断裂处置**：3 个 **删除**（`test_layer1_includes_get_price_pivots` / `test_prompt_contains_missing_tool_guidance` / `test_prompt_set_next_wake_one_shot`）+ 1 个 **缩减**（`test_prompt_contains_memory_quality_guidance` 保留 `actionable` 断言守护新 L28，函数数不变）+ 1 个 **改名**（`test_layer1_bullet_count_25` → `_5`，函数数不变）+ 1 个 **改写 + scope 限制**（`test_prompt_contains_layer1_identity`，函数数不变），净 **-3 函数**
- **L27 + L65 软化测试**：新增 2 个（`test_prompt_l27_softened` + `test_prompt_l65_softened`），净 **+2 函数**（详见 §4.2 新增两行）

**汇总**：函数数净 +3 + 18 + 1 - 3 + 2 = **+21 函数**；pytest cases 净 +3 + 27 + 1 - 3 + 2 = **+30 cases**。

**818 + 30 ≈ 848，落 +22-42 范围内**。
...
### 6.6 N8 拆分潜在 latency 回退

原 `get_critical_alerts` 内部用 `asyncio.gather` 并行 fetch announcements + macro_events（tools_perception.py:627-631），单工具调用即拿两路数据。拆分后 agent 同 cycle 调两个工具只能**串行**（pydantic-ai 工具调用顺序受 LLM 决策驱动），**最坏情况 +1 上游 RTT**。

这是 trade-off 而非 bug — 换得：
- 独立降级（一边 outage 不污染另一边输出）
- 独立 docstring（spec §3.1 DRY 收益）
- 独立缓存命中（NewsService 本就独立 method + 独立 cache）

实际影响有限：announcements + macro 都有 cache 命中率高的 upstream（OKX API + ForexFactory CSV），即使串行多数 cycle 仍命中 cache。观察期可监测 `tool_calls.duration_ms` per-tool 分布作 follow-up 决策依据。
