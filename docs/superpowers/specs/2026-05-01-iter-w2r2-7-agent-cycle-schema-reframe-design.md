...
#### Stale 注释清理（顺手，spec v8 校准 P6 加 3 处）
...
| `src/storage/models.py:177` | ToolCall.cycle_id 注释 "应用层软关联 DecisionLog.cycle_id" — R2-7 rename 后过时 | 改为 "AgentCycle.cycle_id" |
| `src/storage/database.py:112` | alembic upgrade 注释 "batch_alter 重建 decision_logs" — R2-7 rename 后需补 chain 演进指针 | 加注 "decision_logs (Iter 3) → agent_cycles (R2-7 rename)" |
| `src/integrations/exchange/okx.py:320,326` | `_parse_fill_event` 注释引用 `decision_logs / decision_logs.order_id` — decision_logs 本无 order_id 字段（实际指代 `trade_actions.order_id`），R2-7 rename 后双重 stale | 更新为 `trade_actions.order_id`（修本质 stale + R2-7 rename 一并处理）|
...
├── for attempt in range(3):              # 现有 retry loop (cli/app.py:256-292)
│   try: agent.run()
│   ├── except UsageLimitExceeded:
│   │   └── INSERT AgentCycle (forensic: reasoning=NULL, decision=NULL, status='usage_limit_exceeded') — 复用 *_var
│   │       return None
│   └── except Exception (transient): retry
│   else: break
├── (3 次 retry 全失败 → return None, 不写 DB — 见 §6.8)
...
**P2-5 顺序无关注释**: 三步「`trigger_context_var` capture / `state_snapshot_var` capture / prompt 拼接」**互相相对顺序无关**（同源 dataclass 读取 / DB 端镜像 / agent prompt 文本，三条独立路径，D2 已说明）。spec 流程图按"先 capture 后 prompt"列只是叙事方便，impl 时实际顺序由 implementer 决定，唯一约束是**全部都在 retry loop（`for attempt in range(3)`）之前完成**（确保 forensic 路径能复用变量 + 避免每次 retry 重复 capture / IO 倍增）。

### 6.8 Retry-failure 路径处理（P11 校准 spec v8 增补）

`cli/app.py:256-292` 现有 `for attempt in range(3)` retry loop 中，3 次 `agent.run()` 重试都失败（且不是 UsageLimitExceeded） → `return None`。这个路径既不是 success（result 不存在）也不是 forensic（不抛 UsageLimitExceeded）。

**R2-7 决议**: **沿用现状，retry-failure 路径不写 DB**。

理由：
1. spec §6 "前因 → 上下文 → 决策" 叙事的写入只覆盖 success / forensic 二分；retry-failure 不在 R2-7 范围
2. 引入新 `execution_status='retry_failed'` 是新行为改造，违反 R2-7 "纯存储路径 + schema 改造，不动 prompt / agent 行为"纪律
3. 即使 `*_var` 已 in scope（capture 在 retry loop 之前），也**不应**自作主张写入 DB；是否记录 retry-failure 是独立 ops 议题（与持续失败检测同源），留 W2 期间数据驱动启动

implementer **不要**在 retry-failure 路径添加 `AgentCycle.add()` 调用。
...
| `tests/test_usage_limits.py` | DecisionLog → AgentCycle + 字段名改 + 删除 `assert row.decision == "hold"/"open_long"` 类 enum 断言（改为新语义断言）+ **删除 `test_t10_forensic_path_derives_from_committed_trade_actions` 整个测试函数**（P10 校准 spec v8：T10 验证派生函数从 trade_actions 反查写 'open_long' → R2-7 派生函数已删 + forensic decision=NULL，T10 整体失去意义；重构成本高收益低，直接删除）|
...
### §16.8 v8 self-review (2026-05-01, plan 第二轮 audit 触发 spec 同步修订)

#### v8 触发原因

plan v3 reviewer 在第二轮 audit 中发现 spec v7 也有 4 项 ground truth / 表述漏洞（与 plan 联动校准）：

| Issue | 严重度 | spec 涉及位置 |
|---|---|---|
| **P1** order_type 命名错（"stop_loss" vs 实际代码 "stop"）| 🔴 | §4.4 JSON example, §10.2 T-ORD-3 描述 |
| **P6** stale 注释清理范围超出 spec §5.1 授权 | 🟡 | §5.1 仅列 1 处, plan Task 6 扩展到 4 处 |
| **P10** T10 整体删除决议 spec §10.3 未明牌 | 🟢 | §10.3 仅说 "改写"，T10 删除决议在 plan v2 加但 spec 未同步 |
| **P11** retry-failure 路径 spec §6 二分未覆盖 | 🟢 | §6 仅 success / forensic 二分, retry 失败路径 spec 缺 |

代码事实校准（reviewer audit）:
- `simulated.py:51/196/463/489-499` 实测 order_type 用 `"stop"` 不是 `"stop_loss"`
- `okx.py:37/64-69/482/484/545-546` 实测同上
- 全 codebase **没有 "stop_loss"** 字面量

#### v8 修复范围

| 项 | 修复内容 |
|---|---|
| **P1** | §4.4 JSON example `"limit" \| "stop_loss" \| "take_profit"` → `"limit" \| "stop" \| "take_profit"`（与 simulated.py / okx.py order_type 字面量对齐）|
| **P6** | §5.1 stale 注释清理表加 3 处（src/storage/models.py:177 / src/storage/database.py:112 / src/integrations/exchange/okx.py:320,326），与 plan Task 6 范围一致，避免 plan 偏离 spec 授权 |
| **P10** | §10.3 test_usage_limits.py 行加 "**删除 `test_t10_forensic_path_derives_from_committed_trade_actions` 整个测试函数**" 显式条目 + 删除理由 |
| **P11** | §6 加 §6.8 "Retry-failure 路径处理"段：明牌 R2-7 不在该路径写 DB，列 3 条理由（spec §6 二分 / 不引入新 status / 留 W2 数据驱动 ops 议题）|
| (顺手) | §6.7 写入顺序总览图刻意标注 retry loop 嵌套层级（避免 P8 implementer 误把 capture 写到 for loop 内）+ 强化"capture 在 retry loop 之前"约束 |

#### v8 review 状态

✅ P1 (🔴) order_type 命名校准 — spec / plan 与 simulated.py / okx.py 字面量对齐
✅ P6 (🟡) stale 注释清理 spec / plan 一致 — 4 处共同范围
✅ P10 (🟢) T10 删除决议 spec / plan 同步明牌
✅ P11 (🟢) retry-failure 路径 spec §6.8 显式覆盖

spec v8 准备好让用户审阅 + plan v4 同步修订。
