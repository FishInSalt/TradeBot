...
3. The five distance-label sites (six emit points — OCO renders 2) swap `"from current"` → `"from {trigger_ref} price"` where `trigger_ref` is read from a new `BaseExchange.algo_trigger_reference: str = "last"` class attribute (runtime-output single source of truth; OKX="last", Sim="last", future exchanges can override).
4. SimulatedExchange `get_mark_price` reads `self._latest_ticker.last` directly (sim has one price feed; preserves test fixture compatibility; one less indirection than calling `fetch_ticker`).
...
- "Algo trigger = last" is OKX-specific in spirit but **also coincidentally matches Binance USDT-M API default** (per corrected table above). The `algo_trigger_reference` attribute on `BaseExchange` makes this an exchange-level concern; the five distance-label sites (six emit points — OCO renders 2) read from this attribute. The forward-compat lever bites hardest for **Bybit V5**, which has no silent default — `BybitExchange.algo_trigger_reference` must be set to one of `last`/`mark`/`index` explicitly during integration; without the abstraction this becomes five synchronized cross-file text edits. **Hyperliquid / dYdX** use oracle/mark uniformly — `HyperliquidExchange.algo_trigger_reference = "mark"` (or `"oracle"` if we add that token) flips all five output sites at once. The Binance line is therefore not the strongest forward-compat justification; Bybit + Hyperliquid are.
...
| API impl | `SimulatedExchange.get_mark_price` (`src/integrations/exchange/simulated.py`) | Reads `self._latest_ticker.last` directly (sim has single price feed); raises `RuntimeError` if `_latest_ticker is None`. Bypasses `fetch_ticker` indirection — idempotent regardless of whether `fetch_ticker` is observation-only or evolves to mutate state (e.g., synthetic tick advancement for replay scenarios). Net: one less attribute hop than the alternative `await self.fetch_ticker(...)` form, and resilient to future `fetch_ticker` semantics changes. |
...
- **Principle 3 (signal single source)**: Mark and last become two clearly-distinguished sources on the Risk Exposure section (label + value); Liquidation distance becomes single-anchor (mark only). `algo_trigger_reference` is a single source of truth feeding the **runtime output text** at five label sites (six emit points — OCO renders 2) — agents never see the literal trigger reference word (e.g., `"last"`) hardcoded across multiple output sites. Wrapper docstrings (build-time text describing current behavior) describe the OKX-specific concrete reference; if a future Binance integration ships, both the attribute override and the docstrings move together in the same PR. Two layers (runtime output / build-time docstring) intentionally distinguished; single-source applies to the runtime layer.
- **Principle 3 — forward-compat investment honesty**: `algo_trigger_reference` is a single class attribute on `BaseExchange` with one current value (`"last"`). Today it's effectively a named constant; its value as a single-source abstraction is realized only when a second exchange wrapper exists. The strongest payoff cases are **Bybit V5** (no silent default — `triggerBy` must be set explicitly; without this attribute, a Bybit wrapper would force five synchronized cross-file text edits) and **Hyperliquid / dYdX** (oracle/mark-uniform — single-attribute override flips all five label sites). Binance USDT-M is **not** a strong forward-compat case: API default `CONTRACT_PRICE` (= last) coincides with OKX, so a BinanceExchange wrapper would inherit `algo_trigger_reference = "last"` without override. Cost is small (1 attribute); benefit is forward-looking. This is intentional, not over-engineering: the alternative — hardcoding `"last"` at five output sites — would force a synchronized cross-file text edit on the first exchange whose `algo_trigger_reference` differs from OKX's, which is the failure mode this attribute exists to avoid.
...
| `test_algo_trigger_reference_drives_label_text` | Monkey-patch a Sim exchange instance with `algo_trigger_reference = "mark"`; assert the five label sites output `"from mark price"` — confirms single-source-of-truth wiring; this test exists specifically to fail loudly if a future contributor hardcodes `"last"` at any of the five sites |
...
| `tests/test_display_cycle.py` | 11 `"from current"` hits | Cycle-level display snapshots covering all five affected output sites |
...
**Rollback unit**: single PR (spec + plan + impl in three commits per `feedback_plan_doc_commit_first`). Revertable independently. The exchange-API addition is additive (no breakage to existing call sites); the five label-text changes are independently revertable per file.
