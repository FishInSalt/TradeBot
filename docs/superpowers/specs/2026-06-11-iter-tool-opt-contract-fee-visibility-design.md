...
- base 增量：persona Layer 1 +1 行（两句）+ Layer 2 +1 句 + docstring（**三工具**：open_position / place_limit_order / get_position）≈ **~180 tokens/cycle**（实测 cl100k_base：persona ~50 + 三 docstring ~130，其中 open_position 的 inline fee 事实示例占大头 ~80）。属名义 per-cycle prompt/tool-def token（每 cycle 随 system prompt + tool defs 发送、边际由 prompt cache 摊销）
- 变动增量：get_position 每次调用净 +~30-45 chars（Notional 行四因子分解 +~44，Entry fee 行因式化后反而略短）
