...
drift-guard（沿用本仓 persona/docstring drift-guard pattern）：断言 `build_system_prompt` 输出含改动 1 的关键短语（如 `one-shot` / `cancels`），并验证 `wake_max_minutes` 插值未被破坏。非行为测试——真实验证为下个 sim 的 narrative grep + ok-cycle 遗忘率（会话内口径，不跨 sim 裸比；cross-sim 行情不可比）。
