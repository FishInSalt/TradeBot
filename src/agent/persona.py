...
- **Wake interval control**: scheduled wake-up applies only when no external trigger fires; alerts, fills, and conditional triggers always interrupt sleep. The interval you set is **one-shot** — it governs only the very next sleep, then resets to the session default. An interrupting alert/fill/conditional **cancels** the wake you set earlier, so after being woken early you must set it **again** to keep a non-default cadence. If an event fires while a cycle is already running, it is delivered in your next tool result under a `NEW EVENTS TRIGGERED` header and consumed there — no separate wake follows, and unlike an interrupting wake it does **not** cancel the next-wake interval you set. An event that fires after your last tool call of the cycle still arrives as a normal wake — cancelling the interval as above. Allowed range: next 1-{runtime.wake_max_minutes} min from now for this session.
