...
async def test_init_db_path_3_for_empty_db(tmp_path: Path) -> None:
    ...
    R2-7 (T5 / C1 (A2)): assertions retargeted to agent_cycles + new field names.
    init_db path-3 stamps directly to R2-7 head (无视中间 revisions),
    ...
    # 1. Schema completeness — R2-7 agent_cycles 11 字段
    tables = {r[0] for r in cur.execute("SELECT name FROM sqlite_master WHERE type='table'")}
    assert "agent_cycles" in tables, f"agent_cycles 表应存在, tables={tables}"
    assert "decision_logs" not in tables, f"decision_logs 应已 rename, tables={tables}"
    ...
    assert set(cols.keys()) == expected_cols, \
        f"agent_cycles 列集合不匹配, 差异={set(cols.keys()) ^ expected_cols}"

    # decision: Text NULLABLE (R2-7: was VARCHAR(30) NOT NULL)
    assert cols["decision"][2] == "TEXT", f"decision 应 TEXT, got {cols['decision'][2]}"
    assert cols["decision"][3] == 0, f"decision 应 NULLABLE, got notnull={cols['decision'][3]}"
    # execution_status: NOT NULL with server_default 'ok' (R2-7 §6.5)
    assert cols["execution_status"][3] == 1, "execution_status 应 NOT NULL"
    # state_snapshot: Text NULLABLE
    assert cols["state_snapshot"][2] == "TEXT"
    assert cols["state_snapshot"][3] == 0

    # tool_calls / trade_actions 不变
    ...
    # Index rename (R2-7): ix_decision_logs_* → ix_agent_cycles_*
    indexes = {
...
def test_upgrade_from_w1_like_data(tmp_path: Path, alembic_cfg_factory) -> None:
    ...
    # C1 (A1): bind to R2-4 head — this test verifies Iter 3 + R2-4 step semantics
    # (batch_alter merge + decision/status backfill); R2-7 post-state covered
    # separately by T-MIG-1~8 (lines 380-585).
    command.upgrade(cfg, "e7b2bd73c131")  # was "head"; R2-7 renames decision_logs → agent_cycles
...
def test_downgrade_then_upgrade(tmp_path: Path, alembic_cfg_factory) -> None:
    """From W1-like fixture: upgrade R2-4 → downgrade -1 → upgrade R2-4 reentrant + idempotent.

    C1 (A1): bind to R2-4 head ('e7b2bd73c131'); this test verifies Iter 3 + R2-4
    step reentrancy (batch_alter idempotency, no NULL-decision rows lost).
    R2-7 reentrancy is implicitly covered by T-MIG-7 (downgrade路径) +
    test_upgrade_when_already_head (head no-op).
    """
    db_path = tmp_path / "reentrant.db"
    _create_pre_alembic_schema(db_path)
    cfg = alembic_cfg_factory(db_path)

    command.upgrade(cfg, "e7b2bd73c131")  # was "head"; R2-7 renames decision_logs
    command.downgrade(cfg, "-1")
    command.upgrade(cfg, "e7b2bd73c131")  # was "head"

    # Verify final state has all new fields (R2-4 + Iter 3)
    conn = sqlite3.connect(db_path)
    cur = conn.cursor()
...
def test_r2_4_upgrade_widens_decision_logs_decision(tmp_path: Path, alembic_cfg_factory):
    ...
    C1 (A1): bind to R2-4 head — this test verifies R2-4 step semantic
    invariant; R2-7 post-state (decision Text NULLABLE) covered by T-MIG-3.
    """
    db_path = tmp_path / "test.db"
    _create_pre_alembic_schema(db_path)
    cfg = alembic_cfg_factory(db_path)
    command.upgrade(cfg, "e7b2bd73c131")  # was "head"; R2-7 widens decision to Text
...
def test_r2_4_upgrade_preserves_historical_adjust_rows(tmp_path: Path, alembic_cfg_factory):
    ...
    # 跑 R2-4 upgrade
    # C1 (A1): bind to R2-4 head — verify R2-4 step preserves 'adjust' historical
    # rows. R2-7 post-state (decision_logs → agent_cycles rename, decision 字面量
    # 保留) covered by T-MIG-6 (lines 473-524).
    command.upgrade(cfg, "e7b2bd73c131")  # was "head"; R2-7 renames table
...
def test_r2_4_upgrade_preserves_existing_indexes(tmp_path: Path, alembic_cfg_factory):
    ...
    C1 (A1): bind to R2-4 head — verify R2-4 doesn't touch Iter 3 indexes.
    R2-7 index rename (ix_decision_logs_* → ix_agent_cycles_*) covered by T-MIG-5.
    """
    db_path = tmp_path / "test.db"
    _create_pre_alembic_schema(db_path)
    cfg = alembic_cfg_factory(db_path)
    command.upgrade(cfg, "e7b2bd73c131")  # was "head"; R2-7 renames index too
