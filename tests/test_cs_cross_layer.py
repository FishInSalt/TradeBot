...
Guards that the execution-layer cached cs (via _init_contract_size) and the
...
async def test_exec_cs_matches_marketdata_source():
    """Execution-layer cached cs (_init_contract_size) and market-data layer live-read cs share the same source.
    ...
    # Execution layer: _init_contract_size loads markets then caches from market()
